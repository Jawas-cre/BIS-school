  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-3 gap-3 sm:max-w-xl">
        {[
          [V.statWords, totalWords],
          [V.statMastered, mastered],
          [V.statDue, due],
        ].map(([label, value]) => (
      ...
      {decks.length === 0 ? (
        <EmptyState icon={<Languages className="size-5" />} title={V.noDecks}>
          {V.noDecksText}
        </EmptyState>
      ) : (
            ...
            return (
              <Link key={d.id} href={`/vocabulary/${d.id}`} className="pressable-card group flex flex-col rounded-2xl border border-line bg-surface p-5 shadow-card hover:border-line-strong">
                <div className="flex items-center justify-between">
                  ...
                  {dueHere > 0 && <Badge tone="danger">{fmt(V.dueCount, { n: dueHere })}</Badge>}
                </div>
                <h3 className="mt-3 font-display text-lg font-bold group-hover:text-brand">{d.title}</h3>
                <p className="mt-1 flex-1 text-sm text-muted">{d.description}</p>
                <div className="mt-4">
                  <div className="mb-1.5 flex justify-between text-xs text-muted">
                    <span>{fmt(V.deckStats, { mastered: learned, learning, fresh })}</span>
